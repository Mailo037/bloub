...
export function arcRender(seed: ArcSeed, t: number, scale: number, id: string, opacity = 1): ArcRender {
  ...
  for (let i = 0; i <= N; i++) {
    ...
    if (prev !== null && behind !== prev && Math.abs(z - prevZ) > 0.0001) {
      // Exakte Horizont-Schnittstelle (z = 0) nahtlos in beide Pfade einfügen
      const frac = -prevZ / (z - prevZ)
      const hx = r2((prevX + (x - prevX) * frac) * scale)
      const hy = r2((prevY + (y - prevY) * frac) * scale)
      if (behind) {
        front += `L${hx} ${hy}`
        back += `M${hx} ${hy}`
      } else {
